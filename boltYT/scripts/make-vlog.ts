...
async function runComfy(workflow: unknown, outPath: string): Promise<string> {
	...
	for (let i = 0; i < 600; i++) {
		...
		if (!h.ok) continue;
		const rec = ((await h.json()) as Record<string, { outputs?: Record<string, { images?: { filename: string; subfolder: string; type: string }[] }> }>)[prompt_id];
		const img = rec?.outputs ? Object.values(rec.outputs)[0]?.images?.[0] : undefined;
		if (img) {
			const v = await fetch(`${COMFY}/view?filename=${img.filename}&subfolder=${img.subfolder}&type=${img.type}`);
...
function portraitWorkflow(prompt: string, seed: number) {
	return {
		"3": { class_type: "KSampler", inputs: { seed, steps: 30, cfg: 6.5, sampler_name: "dpmpp_2m", scheduler: "karras", denoise: 1, model: ["4", 0], positive: ["6", 0], negative: ["7", 0], latent_image: ["5", 0] } },
		"4": { class_type: "CheckpointLoaderSimple", inputs: { ckpt_name: CKPT } },
		"5": { class_type: "EmptyLatentImage", inputs: { width: 832, height: 1024, batch_size: 1 } },
		"6": { class_type: "CLIPTextEncode", inputs: { text: prompt, clip: ["4", 1] } },
		"7": { class_type: "CLIPTextEncode", inputs: { text: "ugly, blurry, low quality, deformed, multiple people, cartoon, text, watermark", clip: ["4", 1] } },
		"8": { class_type: "VAEDecode", inputs: { samples: ["3", 0], vae: ["4", 2] } },
		"9": { class_type: "SaveImage", inputs: { filename_prefix: "vlog_host", images: ["8", 0] } },
	};
}
...
function sceneWorkflow(prompt: string, seed: number, ref: string) {
	return {
		"4": { class_type: "CheckpointLoaderSimple", inputs: { ckpt_name: CKPT } },
		"10": { class_type: "IPAdapterModelLoader", inputs: { ipadapter_file: "sdxl_models/ip-adapter-plus_sdxl_vit-h.safetensors" } },
		...
		"13": { class_type: "IPAdapterAdvanced", inputs: { model: ["4", 0], ipadapter: ["10", 0], image: ["12", 0], clip_vision: ["11", 0], weight: 0.6, weight_type: "ease in-out", combine_embeds: "concat", start_at: 0, end_at: 0.7, embeds_scaling: "V only" } },
		"5": { class_type: "EmptyLatentImage", inputs: { width: 1344, height: 768, batch_size: 1 } },
		"6": { class_type: "CLIPTextEncode", inputs: { text: prompt, clip: ["4", 1] } },
		"7": { class_type: "CLIPTextEncode", inputs: { text: "ugly, blurry, low quality, deformed, multiple people, different face, cartoon, text, watermark, extreme close-up, cropped face", clip: ["4", 1] } },
		"3": { class_type: "KSampler", inputs: { seed, steps: 30, cfg: 6.5, sampler_name: "dpmpp_2m", scheduler: "karras", denoise: 1, model: ["13", 0], positive: ["6", 0], negative: ["7", 0], latent_image: ["5", 0] } },
		"8": { class_type: "VAEDecode", inputs: { samples: ["3", 0], vae: ["4", 2] } },
		"9": { class_type: "SaveImage", inputs: { filename_prefix: "vlog_scene", images: ["8", 0] } },
	};
}
...
async function ensureHostReference(host: HostCharacter, dir: string): Promise<string> {
	...
	const portrait = await runComfy(portraitWorkflow(photoreal(buildHostReferencePrompt(id)), id.styleSeed % 1_000_000), join(dir, "host.png"));
	const face = join(dir, "host_face.png");
	await exec("sips", ["-c", "620", "560", "--cropOffset", "0", "120", portrait, "--out", face]);
...
async function main(): Promise<void> {
	...
	const stamp = Number(process.env.SOURCE_DATE_EPOCH) || Math.floor(Date.now() / 1000);
	const outDir = args.out ?? join(process.cwd(), "renders");
	mkdirSync(outDir, { recursive: true });
	const work = join(outDir, `vlog_${era.id}_${stamp}`);
	mkdirSync(work, { recursive: true });
	log(`▶ ${era.subjectKo} 시간여행 브이로그 (${sceneCount}씬) — 채널 ${channel}`);
	...
	const cr = await fetch(`${PROXY}/api/openai/chat`, {
		...
		body: JSON.stringify({ messages: [{ role: "system", content: "한국 유튜브 시간여행 역사 브이로그 작가. JSON만 출력." }, { role: "user", content: usr }], response_format: { type: "json_object" } }),
	});
	if (!cr.ok) throw new Error(`대본 ${cr.status} (api-proxy LLM_BACKEND=claude 확인)`);
	const scenes = (JSON.parse((await cr.json()).choices[0].message.content).scenes as { narration: string; visual: string }[]).slice(0, sceneCount);
	log(`   ${scenes.length}씬: ${scenes.map((s) => s.narration.slice(0, 16)).join(" / ")}`);
	...
	for (let i = 0; i < scenes.length; i++) {
		...
		const img = await runComfy(sceneWorkflow(photoreal(buildPovVisualPrompt(scenes[i].visual, era)), 1000 + i * 137, ref), join(work, `scene${i}.png`));
		const mp3 = join(work, `scene${i}.mp3`);
		await tts(scenes[i].narration, mp3);
		const d = await dur(mp3);
		const clip = join(work, `clip${i}.mp4`);
		await exec("ffmpeg", ["-y", "-loop", "1", "-i", img, "-i", mp3, "-filter_complex", `[0:v]scale=${W}:${H}:force_original_aspect_ratio=increase,crop=${W}:${H},zoompan=z='min(zoom+0.0004,1.07)':d=${Math.round(d * 30)}:s=${W}x${H}:fps=30,setsar=1[v]`, "-map", "[v]", "-map", "1:a", "-t", String(d), "-c:v", "libx264", "-pix_fmt", "yuv420p", "-c:a", "aac", "-b:a", "128k", clip]);
		clips.push(clip);
	...
	// 4) 합성 + 소프트 한글 자막
	log("4) 합성 + 자막...");
	writeFileSync(join(work, "concat.txt"), clips.map((c) => `file '${c}'`).join("\n"));
	const merged = join(work, "merged.mp4");
	await exec("ffmpeg", ["-y", "-f", "concat", "-safe", "0", "-i", join(work, "concat.txt"), "-c", "copy", merged]);
	...
	try {
		await exec("ffmpeg", ["-y", "-i", merged, "-i", srtPath, "-map", "0:v", "-map", "0:a", "-map", "1", "-c:v", "copy", "-c:a", "copy", "-c:s", "mov_text", "-metadata:s:s:0", "language=kor", finalPath]);
	} catch {
